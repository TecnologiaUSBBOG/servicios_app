...
import 'package:servicios/screens/estudiante/icetex_e2.dart';
...
class IcetexEScreenE extends StatefulWidget {
  const IcetexEScreenE({super.key});
...
class _IcetexEScreenEState extends State<IcetexEScreenE> {
    ...
    try {
      ...
      setState(() {
        ...
        if (_opcionNivelAcademicoList.isNotEmpty) {
          selectedOpcionLevel =
              _opcionNivelAcademicoList[0]['id']; // Asignar el primer ID
        } else {
          selectedOpcionLevel =
              ''; // Reiniciar a un valor vacío si no hay opciones
        }
      });
    } catch (e) {
      setState(() {
        _opcionNivelAcademicoList = []; // Limpiar la lista en caso de error
        selectedOpcionLevel = ''; // Reiniciar a un valor vacío en caso de error
      });
    }
  }
  ...
  Future<Map<String, dynamic>> fetchData(String nationalId) async {
    final response = await http.post(
      Uri.parse(
        'http://apps.usbbog.edu.co:8080/prod/usbbogota/icetex/IcetexAutocompletado',
      ),
  ...
  Future<List<Map<String, dynamic>>> getOpcLevel() async {
    final response = await http.post(
      Uri.parse(
          'http://apps.usbbog.edu.co:8080/prod/usbbogota/icetex/IcetexNivelAcademico'),
    );
      ...
      try {
        ...
        if (dataList.isNotEmpty) {
          return dataList.map((data) {
            return {
              'text':
                  data['D']?.toString() ?? 'No disponible', // Manejo de nulos
        ...
        throw Exception(
            'Error al decodificar el JSON: $e'); // Manejo de errores en decodificación
      }
    } else {
      throw Exception(
  ...
  Future<List<Map<String, dynamic>>> getOpcCiclo() async {
    final response = await http.post(
      Uri.parse(
          'http://apps.usbbog.edu.co:8080/prod/usbbogota/CreditoDirectoUSB/CreditoCicloAcademico'),
    );
